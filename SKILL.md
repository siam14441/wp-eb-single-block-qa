...
| **P8 — Functionality & UX** | Every user-facing interaction on the live frontend, realistic content variations, edge states (empty, very long, special characters). **Editor-vs-frontend parity**, explicitly — not just when a setting happens to change: on the P2 fixture's realistic render, diff content, layout, spacing, typography, colors, backgrounds, images and icons between the two (the parity and blank/clipping probes in `references/probes.md` exist for exactly this). Interactive behavior itself is frontend-only — many blocks show a deliberately static, non-interactive preview in the editor canvas by design, so expecting drag/click parity *there* produces false findings; verify it works on the frontend instead. Typos, misalignment, clipping, overflow, broken responsive behavior, dead controls, inconsistent labels. Skip anything with no user-facing impact. |
